 ...
 *
 * 安全策略：
 *   - 模式 A：GitHub URL 需满足 star 门槛 + 安全审查。
 *   - 模式 B：安全审查。
 */
...
function rewriteInstallSkillMetadata(content, skillName) {
...
function installContentMatches(existingContent, sourceContent, skillName) {
  return existingContent === sourceContent
    || existingContent === rewriteInstallSkillMetadata(sourceContent, skillName);
}

function makeConflictSkillName(baseName, agentDir, index = 0) {
  const agentId = sanitizeSkillName(path.basename(agentDir || "")) || "agent";
  const suffix = index === 0 ? agentId : `${agentId}-${index + 1}`;
  const stemMax = Math.max(1, 64 - suffix.length - 1);
  const stem = baseName.slice(0, stemMax).replace(/[-_]+$/, "") || "skill";
  return sanitizeSkillName(`${stem}-${suffix}`) || `skill-${suffix}`;
}

function resolveInstallTarget({ installDir, requestedName, sourceContent, agentDir }) {
  const targetFile = path.join(installDir, requestedName, "SKILL.md");
  if (!fs.existsSync(targetFile)) {
    return { name: requestedName, shouldWrite: true };
  }
  const existing = fs.readFileSync(targetFile, "utf-8");
  if (installContentMatches(existing, sourceContent, requestedName)) {
    return { name: requestedName, shouldWrite: false };
  }

  for (let i = 0; i < 1000; i += 1) {
    const candidate = makeConflictSkillName(requestedName, agentDir, i);
    const candidateFile = path.join(installDir, candidate, "SKILL.md");
    if (!fs.existsSync(candidateFile)) {
      return { name: candidate, shouldWrite: true };
    }
    const candidateExisting = fs.readFileSync(candidateFile, "utf-8");
    if (installContentMatches(candidateExisting, sourceContent, candidate)) {
      return { name: candidate, shouldWrite: false };
    }
  }

  throw new Error(`Cannot find a free skill name for ${requestedName}`);
}
  ...
  return {
    ...
    execute: async (_toolCallId, params, _signal, _onUpdate, ctx) => {
      ...
      if (github_url?.trim()) {
        ...
        // 6. 写入目标技能源目录
        const target = resolveInstallTarget({ installDir, requestedName: name, sourceContent: content, agentDir });
        const normalizedContent = rewriteInstallSkillMetadata(content, target.name);
        const skillDir = path.join(installDir, target.name);
        if (target.shouldWrite) fs.mkdirSync(skillDir, { recursive: true });
        const skillFilePath = path.join(skillDir, "SKILL.md");
        if (target.shouldWrite) fs.writeFileSync(skillFilePath, normalizedContent, "utf-8");
        ...
        // 7. 触发回调
        await onInstalled?.(target.name);

        const safetyNote = safetyPassed ? t("error.installSkillSafetyPassed") : "";
        return {
          content: [{ type: "text", text: t("error.installSkillSuccess", { name: target.name, source: fetchedFrom, stars, reason }) + (safetyNote ? "\n" + safetyNote : "") }],
          details: {
            skillName: target.name,
            stars,
            source: "github",
      ...
      return {
        content: [{ type: "text", text: t("error.installSkillSuccessLocal", { name: target.name, reason }) + (safetyNote2 ? "\n" + safetyNote2 : "") }],
        details: {
          skillName: target.name,
          source: "content",

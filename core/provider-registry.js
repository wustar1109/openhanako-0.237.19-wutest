...
function normalizeProviderUserConfigMap(providers) {
  ...
  for (const [providerId, config] of Object.entries(providers)) {
    ...
    normalized[providerId] = normalizeProviderBaseUrl(providerId, normalizeProviderUserConfig(config));
  }
  return normalized;
}

function normalizeProviderBaseUrl(providerId, config) {
  if (providerId === "mimo" && config?.base_url === MIMO_LEGACY_BASE_URL) {
    return { ...config, base_url: MIMO_TOKEN_PLAN_OPENAI_BASE_URL };
  }
  return config;
}
...
import { mimoPlugin, MIMO_LEGACY_BASE_URL, MIMO_TOKEN_PLAN_OPENAI_BASE_URL } from "../lib/providers/mimo.js";
// 国际

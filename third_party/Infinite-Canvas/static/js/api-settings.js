...
const textImageModelList = document.getElementById('textImageModelList') || document.getElementById('imageModelList');
const imageToImageModelList = document.getElementById('imageToImageModelList');
...
const MS_BUILTIN_TEXT_IMAGE_MODELS = [
    'Tongyi-MAI/Z-Image-Turbo',
    'Qwen/Qwen-Image-2512',
    'black-forest-labs/FLUX.2-klein-9B'
];
const MS_BUILTIN_IMAGE_TO_IMAGE_MODELS = [
    'Qwen/Qwen-Image-Edit-2511'
];
const MS_BUILTIN_IMAGE_MODELS = [
    ...MS_BUILTIN_TEXT_IMAGE_MODELS,
    ...MS_BUILTIN_IMAGE_TO_IMAGE_MODELS
];
...
const IMAGE_TO_IMAGE_MODEL_PATTERNS = [
    /(^|[-_/])image[-_]?edit([-_/]|$)/i,
    /(^|[-_/])edit([-_/]|$)/i,
    /img2img/i,
    /image[-_]?to[-_]?image/i,
    /i2i/i,
    /inpaint/i,
    /outpaint/i,
    /control/i,
...
function isImageToImageModel(model){
    ...
    if(MS_BUILTIN_IMAGE_TO_IMAGE_MODELS.includes(value)) return true;
    return IMAGE_TO_IMAGE_MODEL_PATTERNS.some(pattern => pattern.test(value));
}
function splitConfiguredImageModels(imageModels, textImageModels, imageToImageModels){
    const text = unique(textImageModels || []);
    const imageToImage = unique(imageToImageModels || []);
    const combined = unique(imageModels || []);
    const seed = combined.length ? combined : unique([...text, ...imageToImage]);
    seed.forEach(model => {
        if(text.includes(model) || imageToImage.includes(model)) return;
        if(isImageToImageModel(model)) imageToImage.push(model);
        else text.push(model);
    });
    return {
        textImage: unique(text),
        imageToImage: unique(imageToImage),
        image: unique([...text, ...imageToImage])
    };
}
function normalizeProviderModelBuckets(item){
    if(!item) return item;
    const split = splitConfiguredImageModels(item.image_models || [], item.text_image_models || [], item.image_to_image_models || item.image_edit_models || []);
    item.text_image_models = split.textImage;
    item.image_to_image_models = split.imageToImage;
    item.image_models = split.image;
    return item;
}
function refreshCombinedImageModels(item){
    if(!item) return;
    item.image_models = unique([...(item.text_image_models || []), ...(item.image_to_image_models || [])]);
}
function modelKeyForKind(kind){
    if(kind === 'text_image') return 'text_image_models';
    if(kind === 'image_to_image') return 'image_to_image_models';
    if(kind === 'image') return 'image_models';
    if(kind === 'video') return 'video_models';
    return 'chat_models';
}
function modelListForKind(kind){
    if(kind === 'text_image') return textImageModelList;
    if(kind === 'image_to_image') return imageToImageModelList;
    if(kind === 'image') return textImageModelList;
...
function syncEditor(){
    ...
    refreshCombinedImageModels(item);
...
function renderEditor(){
    const item = provider();
    if(!item) return;
    normalizeProviderModelBuckets(item);
    editorTitle.textContent = item.name || item.id;
    ...
    renderModels('text_image');
    renderModels('image_to_image');
    renderModels('chat');
    renderModels('video');
...
async function testConnection(){
    ...
    try {
        ...
        if(data.ok){
            ...
            const split = splitConfiguredImageModels(data.image_models || [], data.text_image_models || [], data.image_to_image_models || []);
            lastFetchedSuggestion = {
                text_image: new Set(split.textImage),
                image_to_image: new Set(split.imageToImage),
                image: new Set(split.image),
...
function openModelPicker(){
    ...
    normalizeProviderModelBuckets(item);
    const existing = {
        text_image: new Set(item.text_image_models||[]),
        image_to_image: new Set(item.image_to_image_models||[]),
    ...
    const allIds = unique([
        ...lastFetchedAll,
        ...(item.text_image_models||[]),
        ...(item.image_to_image_models||[]),
        ...(item.image_models||[]),
        ...(item.chat_models||[]),
        ...(item.video_models||[])
    ]);
    ...
    allIds.forEach(id => {
        ...
        let cat;
        if(existing.text_image.has(id)) cat = 'text_image';
        else if(existing.image_to_image.has(id)) cat = 'image_to_image';
        else if(existing.video.has(id)) cat = 'video';
        else if(existing.chat.has(id)) cat = 'chat';
        else if(lastFetchedSuggestion?.image_to_image?.has(id)) cat = 'image_to_image';
        else if(lastFetchedSuggestion?.text_image?.has(id)) cat = 'text_image';
        else if(lastFetchedSuggestion?.image?.has(id)) cat = isImageToImageModel(id) ? 'image_to_image' : 'text_image';
        else if(lastFetchedSuggestion?.video?.has(id)) cat = 'video';
        else cat = 'chat';
        pickerState.category[id] = cat;
        // 默认勾选状态：已在用户配置里的 = 勾选；新拉的 = 不勾选（让用户主动选）
        pickerState.selected[id] = existing.text_image.has(id) || existing.image_to_image.has(id) || existing.chat.has(id) || existing.video.has(id);
    });
    // 默认 tab 切回「全部」
...
function renderModelPicker(){
    ...
    const ids = pickerOrder.filter(id => Object.prototype.hasOwnProperty.call(pickerState.category, id));
    // 各分类总数 / 已选数
    const totals = { all: ids.length, text_image:0, image_to_image:0, chat:0, video:0 };
    const selecteds = { all:0, text_image:0, image_to_image:0, chat:0, video:0 };
    ids.forEach(id => {
        const cat = pickerState.category[id];
        if(totals[cat] == null) return;
        totals[cat]++;
    ...
    // 底部汇总
    const sumTextImage = document.getElementById('sumTextImage');
    const sumImageToImage = document.getElementById('sumImageToImage');
    ...
    if(sumTextImage){ sumTextImage.textContent = `文生图 ${selecteds.text_image}`; sumTextImage.classList.toggle('picker-sum-chip-empty', selecteds.text_image === 0); }
    if(sumImageToImage){ sumImageToImage.textContent = `图生图 ${selecteds.image_to_image}`; sumImageToImage.classList.toggle('picker-sum-chip-empty', selecteds.image_to_image === 0); }
...
function applyModelPicker(){
    ...
    const textImage = [], imageToImage = [], chat = [], video = [];
    Object.entries(pickerState.selected).forEach(([id, sel]) => {
        if(!sel) return;
        const cat = pickerState.category[id];
        if(cat === 'text_image') textImage.push(id);
        else if(cat === 'image_to_image') imageToImage.push(id);
        else if(cat === 'video') video.push(id);
        else chat.push(id);
    });
    item.text_image_models = textImage;
    item.image_to_image_models = imageToImage;
    item.image_models = unique([...textImage, ...imageToImage]);
    item.chat_models = chat;
    item.video_models = video;
    renderModels('text_image'); renderModels('image_to_image'); renderModels('chat'); renderModels('video');
    renderMsLoras();
    setStatus(`已应用 · 文生图 ${textImage.length} / 图生图 ${imageToImage.length} / LLM ${chat.length} / 视频 ${video.length}，点保存生效`);
    closeModelPicker();
}
...
function renderModels(kind){
    const item = provider();
    if(kind === 'image') normalizeProviderModelBuckets(item);
    const key = modelKeyForKind(kind);
    const list = modelListForKind(kind);
    if(!list) return;
...
function addMsLora(){
    ...
    item.ms_loras.push({
        id:'',
        name:'',
        target_model: (item.text_image_models || item.image_models || [])[0] || MS_BUILTIN_IMAGE_MODELS[0],
        strength:0.8,
        enabled:true,
        note:''
    });
...
function addProvider(){
    ...
    providers.push({id, name:'API', base_url:'https://', protocol:'openai', image_generation_endpoint:'', image_edit_endpoint:'', enabled:true, primary:false, text_image_models:[], image_to_image_models:[], image_models:[], chat_models:[], video_models:[], has_key:false, key_preview:''});
    selectedId = id;
    renderEditor();
}
...
function addModel(kind){
    ...
    const key = modelKeyForKind(kind);
    item[key] = [...(item[key] || []), ''];
    if(kind === 'text_image' || kind === 'image_to_image' || kind === 'image') refreshCombinedImageModels(item);
    renderModels(kind);
    if(kind === 'text_image' || kind === 'image_to_image' || kind === 'image') renderMsLoras();
}
function updateModel(kind, index, value){
    const item = provider();
    const key = modelKeyForKind(kind);
    item[key][index] = value;
    if(kind === 'text_image' || kind === 'image_to_image' || kind === 'image'){
        refreshCombinedImageModels(item);
        renderMsLoras();
    }
}
...
async function loadProviders(){
    ...
    try {
        ...
        providers = (data.providers || []).map(normalizeProviderModelBuckets);
...
async function saveProviders(){
    ...
    try {
        providers.forEach(item => {
            ...
            item.text_image_models = unique(item.text_image_models || []);
            item.image_to_image_models = unique(item.image_to_image_models || []);
            item.image_models = unique([...(item.text_image_models || []), ...(item.image_to_image_models || [])]);
    ...
    try {
        const res = await fetch('/api/providers', {
            ...
            body:JSON.stringify(providers.map(item => ({
                ...
                primary:false,
                text_image_models:item.text_image_models || [],
                image_to_image_models:item.image_to_image_models || [],
        ...
        const data = await res.json();
        providers = (data.providers || providers).map(normalizeProviderModelBuckets);
        providers.forEach(item => {
            delete item.api_key;

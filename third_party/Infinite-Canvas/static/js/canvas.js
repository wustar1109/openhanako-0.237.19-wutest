...
async function loadConfig(){
    ...
    try {
        ...
        const rhWorkflowIds = (rhProvider?.rh_workflows || [])
            .filter(runningHubWorkflowHasConfig)
...
function runningHubWorkflowHasConfig(entry){
    if(!entry || typeof entry !== 'object') return false;
    if(Array.isArray(entry.fields) && entry.fields.length) return true;
    if(entry.workflowJson && typeof entry.workflowJson === 'object' && Object.keys(entry.workflowJson).length) return true;
    if(entry.raw && typeof entry.raw === 'object'){
        const prompt = entry.raw.prompt || entry.raw.workflowJson;
        if(typeof prompt === 'string' && prompt.trim()) return true;
        if(prompt && typeof prompt === 'object' && Object.keys(prompt).length) return true;
    }
...
async function ensureRunningHubWorkflowConfigForNode(node){
    ...
    if(!workflowId) return null;
    const entry = currentRunningHubWorkflowEntry(node);
    if(!runningHubWorkflowCache[workflowId] && (!entry || runningHubWorkflowHasConfig(entry))){
        try { await ensureRunningHubWorkflow(workflowId); } catch(_) {}
    }
...
async function rhFetchWorkflowInfo(nodeId, showAlert=true){
    ...
    try {
        const entry = currentRunningHubWorkflowEntry(node);
        const saved = (!entry || runningHubWorkflowHasConfig(entry))
            ? await ensureRunningHubWorkflow(node.workflowId.trim())
            : null;
        ...
        if(mode === 'text'){
            ...
            const result = await fetch('/api/generate', {
                ...
                body:JSON.stringify({
                    ...
                    workflow_json:'Z-Image-Yaoguang-Detail.json',
                    type:'zimage',
                    client_id:CLIENT_ID
                })
...
function deleteConnection(id, event){
    ...
    event?.stopPropagation();
    event?.stopImmediatePropagation?.();
    if(!connections.some(c => c.id === id)) return false;
    pushUndo();
...
function linkDeleteButton(connection, a, b){
    ...
    btn.textContent = '×';
    btn.addEventListener('pointerdown', e => deleteConnection(connection.id, e));
    btn.addEventListener('mousedown', e => {
        e.preventDefault();
        e.stopPropagation();
        e.stopImmediatePropagation?.();
    });
    btn.addEventListener('click', e => deleteConnection(connection.id, e));
    return btn;
}
...
function connectionDeleteIdFromPoint(clientX, clientY){
    const direct = document.elementFromPoint(clientX, clientY)?.closest?.('.link-delete');
    if(direct?.dataset.connectionId) return direct.dataset.connectionId;
    let bestId = '';
    let bestDistance = Infinity;
    linkControlsEl.querySelectorAll('.link-delete.visible, .link-delete.hover').forEach(btn => {
        const rect = btn.getBoundingClientRect();
        const pad = Math.max(4, 8 * viewport.scale);
        if(clientX < rect.left - pad || clientX > rect.right + pad || clientY < rect.top - pad || clientY > rect.bottom + pad) return;
        const cx = rect.left + rect.width / 2;
        const cy = rect.top + rect.height / 2;
        const distance = Math.hypot(clientX - cx, clientY - cy);
        if(distance < bestDistance){
            bestDistance = distance;
            bestId = btn.dataset.connectionId || '';
        }
    });
    return bestId;
}
function deleteConnectionFromPointer(e){
    if(!canvas || e.button !== 0) return false;
    const id = connectionDeleteIdFromPoint(e.clientX, e.clientY);
    if(!id) return false;
    return deleteConnection(id, e);
}

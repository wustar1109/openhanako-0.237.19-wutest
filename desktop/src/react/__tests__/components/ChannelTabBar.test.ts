...
import { buildTabList, switchTab } from '../../components/channels/ChannelTabBar';
...
describe('ChannelTabBar switchTab', () => {
  ...
  it('places plugin tabs after channels by default', () => {
    const tabs = buildTabList([{ pluginId: 'demo', title: 'Demo', icon: null, routeUrl: '/', hostCapabilities: [] }], []);
    expect(tabs).toEqual(['chat', 'channels', 'plugin:demo']);
  });

  it('switches to channels and closes active panels', () => {
    useStore.setState({ activePanel: 'activity' } as never);

    switchTab('channels');

    expect(useStore.getState().currentTab).toBe('channels');
    expect(useStore.getState().activePanel).toBeNull();
    expect(localStorage.getItem('hana-tab')).toBe('channels');
  });
});

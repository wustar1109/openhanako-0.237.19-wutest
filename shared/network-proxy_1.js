...
export const FORCED_LOCAL_PROXY_BYPASS = Object.freeze(["localhost", "127.0.0.1", "::1"]);

export const DEFAULT_NETWORK_PROXY_CONFIG = Object.freeze({
  mode: "system",
  httpProxy: "",
  httpsProxy: "",
  wsProxy: "",
...
export function noProxyEntries(noProxy) {
...
export function isNoProxyMatch(targetUrl, noProxy) {
  let parsed;
  try {
...
export function proxyConfigFromEnvironment(env = process.env) {
...
export function resolveProxyForUrl(targetUrl, config, env = process.env) {
...
export function proxyConfigToEnvironment(config, baseEnv = process.env) {
...
export function electronProxyRulesForConfig(config) {
  const normalized = normalizeNetworkProxyConfig(config);
...
export function electronProxyBypassRulesForConfig(config) {
  const normalized = normalizeNetworkProxyConfig(config);

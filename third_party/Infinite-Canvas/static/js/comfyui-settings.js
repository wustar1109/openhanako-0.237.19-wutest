...
function uiText(zh, en){ return currentLang() === 'en' ? en : zh; }
function normalizeWorkflowUploadName(value, fallback='workflow'){
    const raw = String(value || fallback || 'workflow').replace(/\.json$/i, '').trim();
    const clean = raw.replace(/[^\u4e00-\u9fa5A-Za-z0-9_.-]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 80);
    return clean || fallback || `workflow_${Date.now()}`;
}
function openInlineModal({title='', message='', defaultValue='', input=false, confirmText='', cancelText='', danger=false}={}){
    return new Promise(resolve => {
        const backdrop = document.createElement('div');
        backdrop.className = 'inline-modal-backdrop';
        const panel = document.createElement('form');
        panel.className = 'inline-modal';
        const titleEl = document.createElement('div');
        titleEl.className = 'inline-modal-title';
        titleEl.textContent = title;
        panel.appendChild(titleEl);
        if(message){
            const messageEl = document.createElement('div');
            messageEl.className = 'inline-modal-message';
        ...
        if(input){
            ...
            inputEl.className = 'inline-modal-input';
            inputEl.type = 'text';
        ...
        actions.className = 'inline-modal-actions';
        const cancelBtn = document.createElement('button');
        cancelBtn.type = 'button';
        cancelBtn.className = 'inline-modal-btn';
        cancelBtn.textContent = cancelText || uiText('取消', 'Cancel');
        const okBtn = document.createElement('button');
        okBtn.type = 'submit';
        okBtn.className = `inline-modal-btn ${danger ? 'danger' : 'primary'}`;
        okBtn.textContent = confirmText || uiText('确定', 'OK');
...
async function askWorkflowUploadName(defaultName){
    const fallback = normalizeWorkflowUploadName(defaultName);
    const value = await openInlineModal({
        title: uiText('工作流名称', 'Workflow name'),
        message: tr('comfy.namePrompt'),
        defaultValue: fallback,
        input: true,
        confirmText: uiText('上传', 'Upload'),
        cancelText: uiText('取消', 'Cancel'),
    });
    return value == null ? null : normalizeWorkflowUploadName(value, fallback);
}
async function confirmWorkflowDelete(name){
    return openInlineModal({
        title: uiText('删除工作流', 'Delete workflow'),
        message: tf('comfy.deleteConfirm', {name}),
...
async function onUpload(event){
    ...
    try {
        ...
        catch { setStatus(tr('comfy.invalidJson')); return; }
        const baseName = file.name.replace(/\.json$/i, '');
        const inputName = await askWorkflowUploadName(baseName);
        if(!inputName) return;
    ...
    } catch(e){ setStatus(e.message || tr('comfy.uploadFailed')); console.error(e); }
}
...
async function onDelete(){
    ...
    if(!await confirmWorkflowDelete(currentConfig.title || selectedName)) return;
    try {
        ...
        await loadList();
        new BroadcastChannel('studio-api').postMessage({ type: 'workflows-changed' });
    } catch(e){ setStatus(e.message || tr('comfy.deleteFailed')); console.error(e); }
}

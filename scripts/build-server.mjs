...
const infiniteCanvasSrc = path.join(ROOT, "third_party", "Infinite-Canvas");
if (fs.existsSync(infiniteCanvasSrc)) {
  fs.mkdirSync(path.join(outDir, "third_party"), { recursive: true });
  fs.cpSync(infiniteCanvasSrc, path.join(outDir, "third_party", "Infinite-Canvas"), { recursive: true });
  console.log("[build-server]   third_party/Infinite-Canvas/");
}

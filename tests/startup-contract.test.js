...
describe("local startup contract", () => {
  ...
  it("dev Electron launcher passes a dedicated Node runtime to main process", () => {
    ...
    expect(env.HANA_PORT).toBe("0");
  });

  it("keeps explicit dev HANA_PORT overrides while defaulting to an ephemeral port", () => {
    expect(applyDevEnvironment({}).HANA_PORT).toBe("0");
    expect(applyDevEnvironment({ HANA_PORT: "14600" }).HANA_PORT).toBe("14600");
  });

  it("derives the dev HANA_HOME from the project root to isolate cloned worktrees", () => {
    const rootA = path.join(path.parse(ROOT).root, "work", "openhanako-0.237.19");
    const rootB = path.join(path.parse(ROOT).root, "work-test", "openhanako-0.237.19");

    const homeA = defaultDevHanaHome({ projectRoot: rootA });
    const homeB = defaultDevHanaHome({ projectRoot: rootB });

    expect(homeA).not.toBe(homeB);
    expect(path.basename(homeA)).toMatch(/^openhanako-0\.237\.19-[a-f0-9]{10}$/);
    expect(applyDevEnvironment({}, { projectRoot: rootA }).HANA_HOME).toBe(homeA);
  });
